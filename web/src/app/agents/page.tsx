...
const inputCls =
  'w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-[13px] text-slate-700 outline-none transition placeholder:text-slate-300 focus:border-brand-400 focus:ring-2 focus:ring-brand-100';
  ...
  return (
    <div className="flex flex-col gap-5 p-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div>
          <h2 className="text-lg font-bold text-slate-800">营销 Agent 协同台</h2>
          <p className="mt-0.5 text-xs text-slate-400">
            多 Agent 链路（热点捕捉 → 文案生成 → 合规质检）· 先审后发 · 人机协同
          </p>
        </div>
        <Badge tone="slate" className="ml-auto hidden md:inline-flex">
          <Bot size={12} />
          真实调用 POST /ai/v1/marketing/generate
        </Badge>
      </div>
        ...
        <div className="xl:col-span-4">
          ...
          >
            <div className="space-y-4">
              ...
              {/* 渠道选择 */}
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-500">发布渠道</label>
                <div className="flex flex-wrap gap-1.5">
                  {ALL_CHANNELS.map((c) => {
