...
> **更新（第四批后）**：以下条目已在本轮完成，不再属于“未完成”——
> ① 大屏**真实事件流**（`GET /api/v1/events/recent` 轮询 + `/events/stream` SSE，数据源为 Outbox，
>    按主体/授权范围过滤；Web 大屏已展示真实事件，模拟卡片仅在显式开关下作为演练保留）；
> ② H5 **订单列表**（`POST /api/v1/orders/guest/list` 批量凭证校验 + 订单中心列表）；
> ③ **RAG 评测集与门槛**（`app/eval/rag_eval.py` 指标纯函数、`data/eval/rag_eval_set.jsonl`、
>    `scripts/eval_rag.py`、7 例离线单测）；
> ④ **资料来源清单**（`data/raw_docs/sources.json`，未核验资料强制标注 note，并有单测校验）；
> ⑤ 阶段 H 脚本：k6 下单/防超卖/限流压测脚本与角色-会话 Playwright E2E 已提供（**尚未执行**）。
> 下文保留的条目即当前真正的未完成项。
...
- ✅ **真实事件流已实现**（第四批）：`GET /api/v1/events/recent`（游标轮询）与
  `GET /api/v1/events/stream`（SSE，5 分钟自动断开由浏览器重连），数据源为 `t_outbox_event`
  真实事件，按主体与 `t_gov_scope` 授权范围过滤；Web 大屏「业务事件流（真实数据）」卡片使用该接口，
  客户端模拟卡片仅在 `NEXT_PUBLIC_SIMULATED_REALTIME_FEED=true` 时作为演练展示。
...
- ✅ **订单列表已实现**（第四批）：`POST /api/v1/orders/guest/list`（≤20 笔，逐条凭证校验，
  仅返回校验通过的脱敏摘要），H5 订单中心展示「我的订单（本机记录）」并可点选查看详情。
- **售后 / 退款申请**：未实现（依赖支付模块）。
...
- **需求资料导入流程**：`ai-service/data/raw_docs` 有 3 份样例文本，已新增**来源清单**
  `data/raw_docs/sources.json`（未核验资料强制标注 `note`，并有离线单测校验结构）。
  **但官方来源仍未核验**（缺发文单位/发文号/链接/生效期），因此这些资料只能用于检索链路演示，
  不得作为政策依据；核验后须补 `publisher/sourceUrl/issuedDate` 并置 `verified=true`。
- ✅ **RAG 评测集与门槛已建立**（第四批）：`app/eval/rag_eval.py`（recall@k、引用存在率、
  拒答正确率、跨租户泄漏率 + 门槛判定）、`data/eval/rag_eval_set.jsonl`（含应拒答用例）、
  `scripts/eval_rag.py`（连库取数并输出指标与退出码）、7 例离线单测。
  **注意**：评测集当前基于未核验样例资料，仅用于链路回归，不代表资料正确性。
...
- **Playwright E2E**：已新增 `web/e2e/roles-and-session.spec.ts`（未登录守卫、登录失败、
  合作社→/b/merchant 且无治理入口、政府→/b/gov 只读无写按钮、过期会话回登录），
  与既有 `core-flows.spec.ts` 并存；**尚未执行**（需先启动 web/h5 dev server）。
- **压力测试**：已提供 `scripts/loadtest/checkout.js`（幂等/防超卖/限流三场景 + 判定说明）
  与 `scripts/loadtest/README.md`；**尚未执行**，文档中不含任何吞吐量数字。
...
| 11. 大屏事件真实来源，否则隐藏 | ✅（第四批） | 数据源为 Outbox 真实事件（轮询 + SSE）；模拟卡片仅在显式开关下作为演练 |
| 12. 一键启动 + 全类测试 | ⚠️ 部分 | Compose 全栈启动演练、E2E 执行、压测执行仍未跑（脚本已提供） |
